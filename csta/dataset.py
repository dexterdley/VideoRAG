...
# Load Official JSON split
def load_json_split(dataset):
    outputs = []
    with open(f'./splits/{dataset.lower()}_splits.json', 'r', encoding='utf-8') as f:
        splits = json.load(f)
        for split in splits:
            train_videos = split['train_keys']
            test_videos = split['test_keys']
            outputs.append((train_videos,test_videos))
    return outputs
...
def create_dataloader(dataset):
    ...
    splits = load_json_split(dataset=dataset)
